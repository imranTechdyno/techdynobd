                            ...
                            <form action="{{ route('admin.update', @$admin->id) }}" method="post" enctype="multipart/form-data">
                                ...
                                <div class="row">
                                    <div class="form-group col-md-4 mb-4">
                                        <label class=""> {{__('Admin Image')}}</label>
                                    ...
                                    <div class="form-group col-md-6">
                                        ...
                                        <select class="form-control select2" name="role">
                                            ...
                                            @foreach(@$roles as $role)
                                                <option value="{{$role->name}}" @if(@$admin->getRoleNames()[0] == $role->name) selected @endif>{{__($role->name)}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                              
                                    <div class="form-group col-md-12">
                                        <button type="submit" class="btn btn-primary float-right">{{__('Update Admin')}}</button>
                                    </div>
                                </div>
                            </form>
                        </div>
