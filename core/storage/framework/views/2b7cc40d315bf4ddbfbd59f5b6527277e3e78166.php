<header>
    <div class="header-area">
        <nav class="navbar navbar-default container">
            <div class="container-fluid">
                <!-- Brand and toggle get grouped for better mobile display -->
                <div class="navbar-header">
                    <button type="button" class="navbar-toggle collapsed" data-toggle="collapse" data-target="#bs-example-navbar-collapse-1" aria-expanded="false">
                        <span class="sr-only">Toggle navigation</span>
                        <span class="icon-bar"></span>
                        <span class="icon-bar"></span>
                        <span class="icon-bar"></span>
                    </button>
                    <a  href="index.html">
                            <img src="<?php echo e(getFile('logo',@$general->logo)); ?>" class="m_logo" alt="" />
                        </a>
                </div>
        
                <!-- Collect the nav links, forms, and other content for toggling -->
                <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
                    <ul class="nav navbar-nav main-menu">
                        <li class="">
                            <a href="index.html"> Home </a>
                        </li>
                        
                    </ul>
                    <ul class="nav navbar-nav main-menu">
                        <li class="dropdown">
                            <a href="#" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false">About Us <span class="caret"></span></a>
                            <ul class="dropdown-menu">
                                <li><a href="#">About Us</a></li>
                                <li><a href="#">Mission & Vision</a></li>
                                <li><a href="#">Our Approach</a></li>
                                <li><a href="#">Our Team</a></li>
                            </ul>
                        </li>
                    </ul>
                    <ul class="nav navbar-nav main-menu">
                        <li class="dropdown">
                            <a href="#" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false">Sister Concern<span class="caret"></span></a>
                            <ul class="dropdown-menu">
                                <li><a href="#">Service List</a></li>
                                <li><a href="#">Service List Fullwith</a></li>
                                <li><a href="#">Service Single</a></li>
                                <li><a href="#">Service Fullwith</a></li>
                            </ul>
                        </li>
                    </ul>
                    <ul class="nav navbar-nav main-menu">
                        <li>
                            <a href="#"> Products </a>
                        </li>
                    </ul>
                    <ul class="nav navbar-nav main-menu">
                        <li>
                            <a href="#"> Corporate Clients</a>
                        </li>
                    </ul>
                    <ul class="nav navbar-nav main-menu">
                        <li class="dropdown">
                            <a href="#" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false">Media<span class="caret"></span></a>
                            <ul class="dropdown-menu">
                                <li><a href="#">Corporate Clients</a></li>
                                <li><a href="#">Blog Single</a></li>
                                <li><a href="#">Career Single</a></li>
                                <li><a href="#">Shortcodes</a></li>
                                <li><a href="#">404 page</a></li>
                                <li><a href="#">Blank page</a></li>
                            </ul>
                        </li>
                    </ul>
                    <ul class="nav navbar-nav main-menu">
                        <li>
                            <a href="#"> Careers</a>
                        </li>
                    </ul>
                    <ul class="nav navbar-nav main-menu">
                        <li>
                            <a href="#"> Contact Us</a>
                        </li>
                    </ul>
                </div><!-- /.navbar-collapse -->
            </div><!-- /.container-fluid -->
        </nav>
        
    </div>
</header>
<!-- header section close --><?php /**PATH C:\laragon\www\tdbdltd\core\resources\views/frontend/layout/header.blade.php ENDPATH**/ ?>